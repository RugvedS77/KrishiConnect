...
import { Loader2, X, CheckCircle, Calendar, MapPin, ArrowRight, RefreshCw, Truck } from 'lucide-react';
            ...
            <div className="flex justify-between items-center">
                {statuses.map((s, index) => (
                    <div key={s} className="flex-1 text-center">
                        <div className={`mx-auto w-3 h-3 rounded-full ${index <= currentStatusIndex ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        <p className={`mt-1 text-[10px] font-medium ${index <= currentStatusIndex ? 'text-green-600' : 'text-gray-400'}`}>{s}</p>
                    </div>
                ))}
            </div>
...
const InfoPill = ({ icon, text, color = "gray" }) => {
    const colorMap = {
    ...
    return (
        <div className={`flex items-center text-sm px-3 py-1 rounded-full ${colorMap[color]}`}>
            {icon}
...
const BookingModal = ({ contract, milestone, onClose, onBookingSuccess, token }) => {
    ...
    const handleGetQuote = async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error("Failed to get quote from server.");
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
                ...
                <div className="p-5 space-y-4">
                    <div>
                        <p className="font-semibold text-green-700 flex items-center">
                            <Truck size={18} className="mr-2 text-green-600" /> {contract.listing.crop_type}
                        </p>
                ...
                <footer className="p-4 bg-gray-50 flex justify-end gap-3 rounded-b-xl">
                    {!quote ? (
                        <button onClick={handleGetQuote} disabled={isLoading} className="px-5 py-2 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 disabled:bg-gray-400">
...
export default function FarmerLogisticsView({ contracts, setContracts, token }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-2">Your Contracts - Logistics</h2>

            {contracts.length > 0 ? (
                contracts.map(contract => (
                    <div key={contract.id} className="bg-green-50/50 p-4 rounded-xl shadow-sm hover:shadow-md transition">
                        <div className="flex justify-between items-start">
                            <div>
                                <h3 className="font-bold text-lg text-green-800">{contract.listing.crop_type}</h3>
                                <p className="text-sm text-gray-600">Contract #{contract.id} with {contract.buyer.full_name}</p>
                            </div>
                        </div>
                        ...
                        <div className="mt-3 grid gap-2">
                            {contract.milestones.map(milestone => {
                                const isDeliveryMilestone =
                                    milestone.name.toLowerCase().includes('harvest') ||
                                    milestone.name.toLowerCase().includes('delivery');

                                return (
                                    <div
                                        key={milestone.id}
                                        className="flex justify-between items-center bg-white px-3 py-2 rounded-lg shadow-sm hover:bg-green-50 transition"
                                    >
                                        <p className="font-medium text-gray-700 text-sm">{milestone.name}</p>

                                        {milestone.shipment ? (
                                            <div className="flex items-center gap-2">
                                                <InfoPill icon={<CheckCircle size={16} className="text-green-600" />} text="Booked" color="green" />
                                                <button
                                                    onClick={() => handleTrackStatus(milestone.shipment.id)}
                                                    disabled={trackingId === milestone.shipment.id}
                                                    className="p-2 text-gray-500 hover:text-green-600"
                                                >
                                                    {trackingId === milestone.shipment.id ? (
                                            ...
                                            <button
                                                onClick={() => setModalData({ contract, milestone })}
                                                className="px-4 py-1.5 text-xs bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700"
                                            >
                                                Book Transport
                                            </button>
                                        ) : (
