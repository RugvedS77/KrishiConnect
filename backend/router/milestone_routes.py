...
# ):
#     # This auth check is for the BUYER, who is creating the contract terms
...
#     # Security check: Only the buyer of THIS contract can add milestones to it
#     if not contract or contract.buyer_id != buyer.id:
#         raise HTTPException(status_code=403, detail="You are not authorized to add milestones to this contract.")
...
#         ai_notes=None,
#         is_complete=False,      # MUST be False on creation
#         payment_released=False  # MUST be False on creation
#     )
#     db.add(new_milestone)
    
#     # --- ALL of the "if contract.payment_terms == 'milestone':" logic has been DELETED ---
#     # This is correct. Payment logic does NOT belong here.
...
#     # 4. Check if all milestones for this contract are now paid
#     #    (We query again AFTER the commit, just to be safe)
#     remaining_unpaid_count = db.query(MilestoneModel).filter(
#         MilestoneModel.contract_id == milestone.contract_id,
#         MilestoneModel.payment_released == False
#     ).count()

#     if remaining_unpaid_count == 0:
#         # All milestones are paid! Update the parent Contract's status
#         print(f"Contract {milestone.contract_id} is now complete. Updating status.")
...
# # In router/milestone_routes.py

# # Make sure MilestoneFarmerUpdate is imported from your schema file
# from schemas.all_schema import MilestoneFarmerUpdate 
